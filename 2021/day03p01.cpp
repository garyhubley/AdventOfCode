...
void day03Part01() {
	...
	// epsilon is the 12 bit complement of gamma
	epsilon = (~gamma) & 0xFFF;

	std::cout << "num lines: " << totalLines << std::endl;
	std::cout << "ones: \n  {";
	for (auto n : numOnes) {
		std::cout << n << ", ";
	}
	std::cout << "}" << std::endl;
	std::cout << "gamma: 0x" << std::hex << gamma << std::endl;
	std::cout << "epsilon: 0x" << std::hex << epsilon << std::endl;
	std::cout.flags( f );

	std::cout << "product: " << epsilon * gamma << std::endl;
}
